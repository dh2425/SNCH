...
def save_checkpoints(self):
    ...
    if self.task ==1:
        ...
        obj = {
        'FusionTransformer': self.FuseTrans.state_dict(),
        'ImageInstanceMlp': self.ImageInstanceMlp.state_dict(),
        'TextInstanceMlp': self.TextInstanceMlp.state_dict()
        }
...
def load_checkpoints(self, file_name):
    ...
    if self.task==2:
        ...
        self.FuseTrans.load_state_dict(obj['FusionTransformer'])
        self.ImageInstanceMlp.load_state_dict(obj['ImageInstanceMlp'])
        self.TextInstanceMlp.load_state_dict(obj['TextInstanceMlp'])
...
def Nor(W):
    ...
    adj = adj_normalizer(adj)
    # adj = adj.astype("float")# torch.float(adj)
    adj = sparse_mx_to_torch_sparse_tensor(adj).float().to(0)
    return adj
