...
lab_path="data/txt/all_labels.txt"
...
def generate_noise_F(noise):
    noise_rate = noise
    data = np.loadtxt(lab_path, dtype=np.float32)  # ndarray: (N, 24)
    for i in noise_rate:
        # 添加噪声
...
def generate_noise_N(noise):
    ...
    for i in noise_rate:
        ...
        output_file = h5py.File('./noise/nus_21-wide-tc21-lall-noise_{}.h5'.format(i), 'w')
